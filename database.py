...
def init_db():
    with sqlite3.connect(DB_NAME) as conn:
        cursor = conn.cursor()
        cursor.execute('CREATE TABLE IF NOT EXISTS gbans (user_id INTEGER PRIMARY KEY, reason TEXT, admin_id INTEGER, date TEXT)')
        cursor.execute('CREATE TABLE IF NOT EXISTS bot_chats (chat_id INTEGER PRIMARY KEY, enforce_gban INTEGER DEFAULT 1)')
...
def remove_from_gban(user_id):
    with sqlite3.connect(DB_NAME) as conn:
        cursor = conn.execute("DELETE FROM gbans WHERE user_id = ?", (user_id,))
        conn.commit()
        return cursor.rowcount > 0

def update_chat_enforcement(chat_id, status):
    with sqlite3.connect(DB_NAME) as conn:
